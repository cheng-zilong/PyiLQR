...
if __name__ == "__main__":
    ...
    for tau in range(T):
        if tau < T:
            additional_obj_parameters_matrix[tau] = np.asarray((10, 0.1, 1, 1, 1, 0.1), dtype = np.float64)
        else: 
            additional_obj_parameters_matrix[tau] = np.asarray((0, 0, 10, 10, 100, 0), dtype = np.float64)
    ...
    nn_dynamic_model.pre_train(dataset_train, dataset_vali, max_epoch=50000, stopping_criterion=1e-4, lr = 0.001, model_name = "CartPole.model")
